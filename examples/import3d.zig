...
fn update_ui(app: *App) void
{
    ...
    const ui_win= struct
    { var open= true; };
    ui_win.open= (nk.nk_begin(app.window.nkx, "Assimp Importing", nk.nk_rect(5, 5, 230, 100),
        nk.NK_WINDOW_BORDER|nk.NK_WINDOW_MOVABLE |
        nk.NK_WINDOW_MINIMIZABLE|nk.NK_WINDOW_TITLE) == 1);
    if(ui_win.open)
    {
        const campos= app.mainCamera.getPosition();

        nk.nk_layout_row_dynamic(ctx, 22.0, 1);
        nk.nk_labelf(ctx, nk.NK_TEXT_LEFT,
            "eyepos: %3.2f, %3.2f, %3.2f", campos[0], campos[1], campos[2]);
        nk.nk_labelf(ctx, nk.NK_TEXT_LEFT, "viewangle: %3.2f, %3.2f", deg(app.mainCamera.yawRad), deg(app.mainCamera.pitchRad));
    }
    nk.nk_end(ctx);
}
...
const IMPORT_FLAGS:u32=
    // ai.aiProcess_FlipUVs |
    ai.aiProcess_Triangulate |
    ai.aiProcess_JoinIdenticalVertices |
    ai.aiProcess_OptimizeMeshes |
    ai.aiProcess_PreTransformVertices
;

fn printNodeHierarchy(n: *ai.aiNode) void
{
    std.log.debug("'{s}'",.{@as([*:0]const u8, @ptrCast(&n.mName.data))});

    for(n.mChildren[0..n.mNumChildren]) |child|
    { printNodeHierarchy(child); }
}

fn importFBX(app: *App) ![]Object
{
    const filepath= "XYZ arrows.FBX";
    const ai_scene = ai.aiImportFile (filepath, IMPORT_FLAGS)
    orelse
    {
        std.log.err("Failed to import '{s}'",.{filepath});
        return error.AssimpImportScene;
    };
    defer ai.aiReleaseImport(ai_scene);

    // printNodeHierarchy(ai_scene.*.mRootNode);
    const ai_meshes= ai_scene.*.mMeshes[0..ai_scene.*.mNumMeshes];

    var meshes: std.ArrayList(Mesh.Object)= try .initCapacity(gpa.allocator(), 8);
    for(ai_meshes) |m|
    {
        var vbuf: [4*1024]f32= undefined;
        var ibuf: [1024]u32= undefined;
        var vertices: std.ArrayList(f32)= .initBuffer(&vbuf);
        var indices: std.ArrayList(u32)= .initBuffer(&ibuf);

        for(m.?.*.mVertices[0..m.?.*.mNumVertices]) |aiVertex|
        {
            const v= vertices.addManyAsArrayAssumeCapacity(3);
            v[0]= aiVertex.x;
            v[1]= aiVertex.y;
            v[2]= aiVertex.z;
        }
        
        for(m.?.*.mFaces[0..m.?.*.mNumFaces]) |mFace|
        {
            std.debug.assert(mFace.mNumIndices == 3);
            const i= indices.addManyAsArrayAssumeCapacity(3);
            i[0]= mFace.mIndices[0];
            i[1]= mFace.mIndices[1];
            i[2]= mFace.mIndices[2];
        }

        const new_mesh= meshes.addOneAssumeCapacity();
        new_mesh.* =try core.Mesh
            .create(.OnlyPosition, vertices.items, indices.items)
            .createObject(&app.glPool);
    }
    
    return meshes.toOwnedSlice(gpa.allocator());
}
...
pub fn appInit(app: *App, system: core.SetupResult) anyerror!void
{
    ...
    std.log.debug("Press Arrow keys for camera controls (orbit)",.{});
    ...
    app.imported= try importFBX(app);
...
pub fn appQuit(app: *App) void
{
    ...
    gpa.allocator().free(app.imported);
...
pub fn appIterate(app: *App) anyerror!bool
{
    ...
    mat_model= zm.mul(mat_model, zm.scalingV(@splat(0.01)));
...
pub fn build(b: *std.Build, src_path: []const u8, opt:anytype) *std.Build.Module
{
    ...
    // Assimp already builds stb_image internally, so core only needs access to the header
    const assimp= b.dependency("zig_assimp",.
        { .target=opt.target, .optimize=opt.optimize, .formats=.FBX });
    core_mod.addIncludePath(assimp.path("contrib/stb"));
    ...
    impl_mod.linkLibrary(assimp.artifact("assimp"));
